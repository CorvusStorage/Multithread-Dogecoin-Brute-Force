...
function credit(){
    ...
    data.toString().split("\n").forEach(address => {
        if (address.startsWith('D')) {
            ...
            console.error('Error: addresses are not in correct format. Addresses must start with D');
            process.exit(1);
        }
    });
    ...
    function generate() {
        ...
        // Generating a new random Dogecoin keypair
        const privateKey = crypto.randomBytes(32);
        const ck = new CoinKey(privateKey, ci('DOGE'));
        const publicKey = ck.publicAddress;
